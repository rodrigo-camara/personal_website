...
function parseCSV(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    const [id, duration, conference, venue, location, season, year, document] = lines[i].split(';');
    talks.push({ id, duration, conference, venue, location, season, year, document });
  }
...
function displayTalks(talks) {
  const talksDiv = document.getElementById('list_talks');
  ...
  if (talksDiv) {
    ...
    talks.forEach(talk => {
      ...
      // creating div element for title of talk
      const div_title = document.createElement('div');
      div_title.className = 'div_list_element_title';
      const p_title = document.createElement('p');
      p_title.textContent = `${talk.id}`
      ...
      anchor_download.href = `./talks_documents/${talk.document}`;
      anchor_download.setAttribute('download', `${talk.document}`);
      div_download.append(anchor_download);

      div.appendChild(div_download);
      
      // creating div element for duration
      const div_duration = document.createElement('div');
      div_duration.className = 'div_list_element_duration';
      const p_duration = document.createElement('p');
      // bold sign for conference
      const bold_duration = document.createElement('b');
      bold_duration.textContent = "Duration: ";
      p_duration.appendChild(bold_duration);
      p_duration.innerHTML += `${talk.duration}`;
      div_duration.appendChild(p_duration);
      ...
      p_conference.innerHTML += `${talk.conference}`;
      ...
      p_info.textContent = `${talk.location}, ${talk.season} ${talk.year}`;
